...
function ResetCategorize(){
  ...
  // ラベルカテゴリ部分のリセット
  // 要素取得
  var original_label_elems = document.getElementsByName("label-in-category-customize");
  // 元々のラベルをすべて表示
  for(var i=0;i<original_label_elems.length;i++){
    original_label_elems[i].parentNode.style.display = "block";
  }
  // 新ラベルリストを空に
  document.getElementById("new-labels-output").innerHTML = "";
...
function CategorizeByColon(){
  ...
  for(var k=0;k<max_loop;k++){
    ...
    document.getElementById("new-label-input").value = new_label_name;
    // カテゴリ化を実行
    Categorize();
  }
  return;
...
function AppearCustomCategorizeForm(){
  var new_label_input_forms = document.getElementsByName("new-label-input-form");
...
function Categorize(){
  var original_label_elems = document.getElementsByName("label-in-category-customize");
  var new_label_name = document.getElementById("new-label-input").value;
...
function UpdateNewLabelsList(generated_labels){
  ...
  document.getElementById("new-labels-output").innerHTML = generated_labels_text;
  return;
}
